...
ga.opt.maxgen_stage = [20];  % her blok için nesil
...
if isfield(ga,'opt') && isfield(ga.opt,'enable') && ga.opt.enable
    ...
    switch ga.design_set
      case 1
        fprintf('[GA] set=1 | pop=%d | gen=%d | bounds(d_o)=%.1f..%.1f mm | Dp=%.0f..%.0f mm | Lgap=%.0f..%.0f mm\n', ...
...
function [lb,ub,int_idx,names] = ga_get_bounds(set_id)
    switch set_id
        case 1
            ...
            lb = [2e-3, 0.02, 1, 1.5e9, 15e-3, 0.06, 20, 0.1, 0.10];
            ub = [6e-3, 0.05, 1.50, 2.0e9, 3e-3, 0.10, 40, 0.14, 0.24];
            int_idx = 7; % n_turn
            names = {'d_o','Lori','mu_ref','Kd','d_w','D_m','n_turn','Dp','Lgap'};

        case 2
            % [n_orf, Cd0, CdInf, Rec, p_exp, cav_sf, Lh, K_leak, resFactor]
            lb = [2, 0.55, 0.90, 2000, 1.0, 0.95, 3e-3, 1e-9,  10];
            ub = [2, 0.70, 1.10, 6000, 1.6, 1.00, 2e-3, 6e-9,  18];
            int_idx = 1; % n_orf
...
function [lb,ub,int_idx,descr] = ga_get_bounds(set_id)
% GA kapalı olsa bile başta çağrılıyor; güvenli sınırlar döndür.
% set=1: geometri/rijitlik (9 değişken: d_o,..., Dp(idx8), Lgap(idx9))
% set=2: hidrolik (9 değişken)
% set=3: termal/akışkan tamamlayıcı (10 değişken)
    switch set_id
        case 1
            % [d_o,  (placeholderlar)  ..., Dp, Lgap]
            lb = [1.5e-3,  0,0,0,0,0,0,  0.06, 0.10];
            ub = [4.0e-3,  1,1,1,1,1,1,  0.16, 0.30];
            int_idx = []; descr = "geom/rigid";
        case 2
            % [n_orf, Cd0, CdInf, Rec, p_exp, cav_sf, Lh,  K_leak,   resFactor]
            lb = [1,      0.50, 0.70, 2000, 0.8, 1.00, 1.0e-3, 0,       8];
            ub = [6,      0.80, 0.95, 6000, 1.6, 1.20, 8.0e-3, 5e-9,   30];
            int_idx = 1;  descr = "hydraulics";
        case 3
            % [n_orf, Cd0, mu_ref(Pa·s), b_mu(1/°C), beta0(Pa), b_beta(1/°C), hA_os(W/K), dP_cap(Pa), Vmin_fac, resFactor]
            lb = [1,      0.50, 0.50, -0.020, 5e8, -0.010,  300,  1e8, 0.90,  8];
            ub = [6,      0.80, 2.50, -0.004, 5e9, -0.001, 3000,  8e8, 0.99, 40];
            int_idx = 1;  descr = "thermal+fluid";
        otherwise
            lb = 0; ub = 1; int_idx = []; descr = "unknown";
    end
end
